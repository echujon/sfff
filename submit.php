<html>
 <head>
 <?php include_once('links.php')?> 
  <title>Submit Your Film</title>
 </head>
 <body>
 	<?php include_once('header.php')?> 
 	<div class="container">
		<div id="content">
			<div class="section">
				<h3>Submitting Your Film</h3>
				<p>
					So you want to submit a film?<br>
					Great, show us what you've got!!!<br> 
					First, make sure you meet these requirements:
					<ul>
						<li>Film must be 7 minutes or less</li>
						<li>Film must be family-friendly</li>
						<li>$15.00 entry fee</li>
					
					</ul>
					Submissions are due by September 15th, 2015.</br>
					For more information about requirements check out our
					film festival page
					on <a href="https://filmfreeway.com/festival/SaltFlatsFilmFestival">FilmFreeway</a>.
				</p>
				<p>
					There are 2 ways you can submit your film:
				</p>
				<h4>DVD Copy</h4>
				<p> Print out this <a href="/salt_flats_film_festival_submission_form.pdf" target="_blank">submission form</a>
				, agree to the terms and conditions, and sign it.</p>
				<p>
				<p>Submit your DVD, submission form, and your entry fee of <br>
				$15.00
				to:
				</p>
				<p>	
					Joy Foundation </br>
					38 East 400 South</br>
					Bountiful, UT 84010</br> 
				</p>
				<h4>FilmFreeway</h4>
				You can also submit you film online on our <a href="https://filmfreeway.com/festival/SaltFlatsFilmFestival">FilmFreeway page</a>
			</div>
		</div>
 	</div>
 	<?php include_once('footer.php')?>
 </body>
</html>